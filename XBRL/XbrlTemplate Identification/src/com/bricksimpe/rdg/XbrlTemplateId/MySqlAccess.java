...
public class MySqlAccess {
	...
	public SubmissionInfo UpdateSubmissionsTable(Connection con, String ExtractFileName,
		...
		try {
			...
			subInfo.setUid(SubUid);
			subInfo.setVersion(Version);
			if(rs.next() == true) {
	...
	public void UpdateNoteTable(Connection con, SubmissionInfo si, int iNoteUid, int TableCounter, int iNoteIndex, 
	    ...
	    try {
	    	query = "Select Uid from TemplateParse where NoteIndex = " + iNoteIndex + " AND SubUid = " +
	                 si.getUid() + " AND Version = " + si.getVersion() + " AND TemplateId = -5 " +
	    			" AND BeginLineNum is NULL Order by UID";
	    	pstmt = con.prepareStatement(query);
	    	rs = pstmt.executeQuery();
	    	if(rs.next()) {
	    		TemplateParseUid = rs.getInt(1);
	    		rs.close();
                query =  "UPDATE TemplateParse  Set  BeginLineNum = " + iTblBeginLine + ", EndLineNum = " + iTblEndLine + 
                        ", HtmlBeginLineNum = " + hfx.getTableBeginLine() + ", HtmlEndLineNum = " + hfx.getEndLine() +
	...
	public ArrayList<TemplateNotes> WriteTemplateInfo(Connection con, SubmissionInfo si, 
		...
		//int        				  templateParseUid = 0;
		TemplateUpdateCls         templateUpdateCls = null;
		//int                       NoteUidNdx = 0;
		
		try {
			//NoteUids = GetNoteUids(con, si);
           ...
           for(int k = 0; k < TemplateTrace.size(); k++) {
               ...
               templateUpdateCls = GetTemplateParseUid(con, si, TemplateTrace.get(k).getTemplateId());  
               if(templateUpdateCls.GetUid() == 0)  { // no open record to write so insert a new record
                   ...
                   query =  "UPDATE TemplateParse  Set  BeginLineNum = " + TemplateTrace.get(k).getBeginLine() +
                            ", EndLineNum = " + TemplateTrace.get(k).getEndLine() + 
                            ", Confidence = " +  TemplateTrace.get(k).getConfidenceLevel() + ", HtmlBeginLineNum = " + beginLineNum +
                            ", HtmlEndLineNum = " + hfx.getEndLine() + // ", IdentifiedText = '" + TagRemoved +
                            ", UnAudited = " + TemplateTrace.get(k).getUnAudited() + ", Scale = " + iScale + 
                            ", IdLine = " + TemplateTrace.get(k).getIdLine() + ", NoteIndex = " + iNoteCounterToWrite +
             	            " where SubUid = " + si.getUid() +" and Version = " + si.getVersion() + " and TemplateId = " +
             	           templateUpdateCls.GetTemplateId() + " And uid = " + templateUpdateCls.GetUid();
               }
               pstmt = con.prepareStatement(query);
               pstmt.execute();
               if(TemplateTrace.get(k).getTemplateId() == 4) {
                	//con.setAutoCommit(false);
                	    identityStr = "Select Uid from TemplateParse where SubUid = " + si.getUid() +
               	    		      " and Version = " + si.getVersion() + " AND TemplateId = 4" +
               	    		      " and BeginLineNum = " + TemplateTrace.get(k).getBeginLine();
                	Uidstmt = con.prepareStatement(identityStr);
                	//pstmt.execute();
    	            rs = Uidstmt.executeQuery();
    	            rs.next();
	...
	private TemplateUpdateCls GetTemplateParseUid(Connection con, SubmissionInfo si, int templateId) {

		TemplateUpdateCls  rtnCls = new TemplateUpdateCls();
		PreparedStatement  pstmt;
		ResultSet          rs;
		String             query;
		Integer            thisRec;
		...
		try {
			rtnCls.SetTemplateId(templateId);
			query = "Select Uid from TemplateParse where SubUid = " + si.getUid() +
					" and Version = " + si.getVersion() + " AND TemplateId = " + templateId +
					" and BeginLineNum is null Order by Uid";
			pstmt = con.prepareStatement(query);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				...
				if(templateId == 7)  { // may be a 5
					rs.close();
					query = "Select Uid from TemplateParse where SubUid = " + si.getUid() +
							" and Version = " + si.getVersion() + " AND TemplateId = 5" + 
							" and BeginLineNum is null Order by Uid";
					pstmt = con.prepareStatement(query);
					rs = pstmt.executeQuery();
					if(rs.next()) {
					    rtnCls.SetUid(rs.getInt(1));
					    rtnCls.SetTemplateId(5);
					}
				}
			}
			rs.close();
			...
			GenericErrorCls(con, "GetNoteUids", "reading notes: " + e.getMessage(), false);
		}
		return(rtnCls);
	}
	...
	public int WriteAppError(Connection con, ErrorCls errorCls) {  //static
        ...
        if(iRtn != 0) {
		    try {
		    	...
		    	pstmt.setString(4,"XbrlTemplateIdentification");
	...
	public void GetItemsIdentified(Connection con, SubmissionInfo si, TemplateLogger me) {
		...
		try {
			query = "Select count(*) from TemplateParse where SubUid = " + si.getUid() + " and Version = " +
		            si.getVersion() + " And TemplateId > 0 And TemplateId != 4";
			pstmt = con.prepareStatement(query);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				...
				rs.close();
				query = "Select count(*) from TemplateParse where SubUid = " + si.getUid() + " and Version = " +
		            si.getVersion() + " And TemplateId = 4 AND BeginLineNum is not null";
				pstmt = con.prepareStatement(query);
				rs = pstmt.executeQuery();
				if(rs.next()) {
