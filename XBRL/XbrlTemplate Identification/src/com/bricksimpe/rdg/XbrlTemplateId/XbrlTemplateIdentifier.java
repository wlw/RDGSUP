...
public class XbrlTemplateIdentifier {
	/**
	 * @param 
	 * @param args
	 * 
    ...
    private int    privSubUid = 0;
    private int    privVersion = 0;
	...
	public int RunThis()
	{
		...
		iRtn = run(getPrivInputFile(), getPrivCreateOutputFile(), 
				...
				getPrivSubUid(), getPrivVersion(), getPrivCon());
	    return(iRtn);
	}
	...
	public static void main(String[] args) {
		...
		String                    UserName = args[6];
		                          Password = args[7];
	    int                       SubUid = Integer.parseInt(args[8]);
	    int                       Version = Integer.parseInt(args[9]);
		boolean useWindows = false;
		if(args.length > 9)
			useWindows = Boolean.valueOf(args[10]);
		XbrlTemplateIdentifier main = new XbrlTemplateIdentifier();
		
		main.run(InputFile, CreateOutputFile, ServerName, Instance, PortNumber,
		    DataBase, UserName, Password, useWindows, SubUid, Version, null);
	}
	...
	private int run(String InputFile, String CreateOutputFile, String ServerName,  // static
			                ...
			                int SubUid, int Version, Connection privCon) {
		...
		try {
		    ...
		    if(con == null) 
		    	...
		    	subInfo = mysq.UpdateSubmissionsTable(con, InputFile, MaxConfidenceNdx, SubUid, Version);
	        			    ...
	        			    switch (iLookFor) {
	        			        ...
	        			        case 1:
	        			    	    ...
	        			    	    if(constants.CheckForIdentifier(curLine, 1) != -1) {
	        			    		    ...
	        			    		    mySql.UpdateNoteTable(con, si, iNoteUid, TableCounter, iNoteIndex, iTblBeginLine, iTblEndLine, bDefinitionTbl);
	        			    		    TableCounter++;
	        			    	    }
	        			    	    break;
	        			    }
	...
	public void setPrivSubUid(int privSubUid) {
		this.privSubUid = privSubUid;
	}
	
	public int getPrivSubUid() {
		return(privSubUid);
	}
	
	public void setPrivVersion(int privVersion) {
		this.privVersion = privVersion;
	}

	public int getPrivVersion() {
		return privVersion;
	}
}
