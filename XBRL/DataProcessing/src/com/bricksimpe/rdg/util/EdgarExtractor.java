...
public class EdgarExtractor {

	private String privInputFile = "";
	private String privOutputFile = "";
	private String privUserCompanyUid = "";
	...
	public int RunThis()
	{
		...
		iRtn  = run(getPrivInputFile(), getPrivUserCompanyUid(),
				getPrivOutputFile(), getPrivPrevUid(), 
				getPrivServerName(), getPrivPortNumber(), 
				getPrivInstance(), getPrivDataBase(), 
				getPrivDisplayName(), getPrivUserName(), 
				getPrivPassword(), getPrivUseWindows(),
				getPrivCompanyName());
		return(iRtn);
	}
	...
	public static void main(String[] args) { 
		...
		EdgarExtractor main = new EdgarExtractor();
		// TODO Auto-generated method stub
		int iRtn = 0;

		String InputFile = args[0];
		String UserCompanyUid = args[1];
		String OutputFile = args[2];
		String PrevUid = args[3];
		String ServerName = args[4];
		String PortNumber = args[5];
		String Instance = args[6];
		String DataBase = args[7];
		String DisplayName = args[8];
		String UserName = args[9];
		String Password = args[10];
		String CompanyName = args[11];
		Boolean useWindows = Boolean.valueOf(args[12]);
		iRtn = main.run(InputFile, UserCompanyUid, OutputFile, PrevUid, ServerName, PortNumber, Instance, 
				   DataBase, DisplayName, UserName, Password, useWindows, CompanyName);
		iRtn = iRtn;   // RETURN CLEAN
	}
	 
	private int run(String InputFile, String UserCompanyUid, String OutputFile, String PrevUid, // static
		...
		int              iUserCompanyUid = 0;
		int              identifiedSource = 0;
		...
		try {
			iUserCompanyUid = Integer.parseInt(UserCompanyUid);
			...
			if(con == null)
		    	...
		    	identifiedSource = identifiedHtmlSource.FindSourceOfHtml(InputFile);
                ...
                if(iPrevUid == 0) {
                	if(iUserCompanyUid == 0)
                        iRtn = FindCompanyName(OutputFile, con, CompanyName, identifiedSource);
                	else
                		iRtn = iUserCompanyUid;
                    if(iRtn > 0) {
	...
	private int FindCompanyName(String InFileName, Connection con, String UserCompanyName, int htmlSrc) { // static
		int        iCompanyId = 0;
        TableUpdate tu = new TableUpdate();
		...
		try {
            ...
            if(CompanyName.length() > 0) {
            	...
            	errorCls.setErrorText("Company Name was not identified");
            	mysq.WriteAppError(con, errorCls);
            }
		}
		catch (Exception e) {
	...
	public void setPrivUserCompanyUid(String privUserCompanyUid) {
		this.privUserCompanyUid = privUserCompanyUid;
	}
	public String getPrivUserCompanyUid() {
		return privUserCompanyUid;
	}
